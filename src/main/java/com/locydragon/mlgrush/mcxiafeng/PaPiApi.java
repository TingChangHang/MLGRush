...
public class PaPiApi extends EZPlaceholderHook{
	...
	public String onPlaceholderRequest(Player p, String params) {
		if(params.equals("CPS")) {
			...
			return InGame.get(p).area.A.getPlayer().getName();
		}
		else if(params.equals("PLAYERB")) {
			return InGame.get(p).area.B.getPlayer().getName();
		}
		else if(params.equals("SCOREA")) {
			return ""+InGame.get(p).area.a_Score;
		}
		else if(params.equals("PLAYERB")) {
			return ""+InGame.get(p).area.b_Score;
		}
