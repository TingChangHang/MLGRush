...
public class BedSetter implements Listener {
    ...
    public void onBreak(BlockBreakEvent e) {
        if (states.containsKey(e.getPlayer().getName().toLowerCase())) {
            ...
            if (area == null) {
                e.getPlayer().sendMessage(PlaceholderAPI.setPlaceholders(e.getPlayer(), SettingManager.lineformessage$CantfindRegion));
            } else {
                ...
                area.saveArea();
                e.getPlayer().sendMessage(PlaceholderAPI.setPlaceholders(e.getPlayer(), SettingManager.lineformessage$SetingSuccess));
            }
        }
    }
